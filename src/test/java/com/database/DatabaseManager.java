...
public class DatabaseManager {
	...
	private static final int MAXIMUM_POOL_SIZE = Integer.parseInt(ConfigManager.getProperty("MAXIMUM_POOL"));
	...
	private static Connection connection;// Any update that happens to this conn variable all threads will be aware of
									// it.

	// No one creates an object outside the class
	...
	public static void initializePool() {
		...
		if (hikariDataSource == null) { // First all the parallel threads will enter
			synchronized (DatabaseManager.class) {
				if (hikariDataSource == null) { // Only for first connection request
					...
					hikariConfig.setConnectionTimeout(CONNECTION_TIMEOUT_IN_SECS);// 10 secs
					hikariConfig.setIdleTimeout(IDLE_TIMEOUT_SECS);
					hikariConfig.setMaxLifetime(MAX_LIFE_TIME_IN_MINS);// 30 mins
	...
	public static Connection getConnection() throws SQLException {
		if (hikariDataSource == null) {
			...
			throw new SQLException("HIKARI DATA SOURC IS CLOSED");
		}
